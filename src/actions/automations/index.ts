...
export const getInstagramProfilePosts = async () => {
  const user = await onCurrentUser();
  try {
...
export const activateAutomation = async (id: string, state: boolean) => {
  ...
  if (state) {
    const canActivate = await canActivateAutomation(id);
    if (!canActivate) {
      return {
        status: 400,
        data: "Automation must have at least one trigger and one listener before activation.",
      };
    }
  }
